...
//		attackArea.height = 36;
	...
	private void getPlayerAttackImage() {
		...
		if (currentWeapon.type == type_axe) {
			// Vertical attacks - normal width, double height
			attackUp1 = setup("/player/attacking/boy_axe_up_1", gp.tileSize, gp.tileSize * 2);
			attackUp2 = setup("/player/attacking/boy_axe_up_2", gp.tileSize, gp.tileSize * 2);

			attackDown1 = setup("/player/attacking/boy_axe_down_1", gp.tileSize, gp.tileSize * 2);
			attackDown2 = setup("/player/attacking/boy_axe_down_2", gp.tileSize, gp.tileSize * 2);

			// Horizontal attacks - double width, normal height
			attackLeft1 = setup("/player/attacking/boy_axe_left_1", gp.tileSize * 2, gp.tileSize);
			attackLeft2 = setup("/player/attacking/boy_axe_left_2", gp.tileSize * 2, gp.tileSize);

			attackRight1 = setup("/player/attacking/boy_axe_right_1", gp.tileSize * 2, gp.tileSize);
			attackRight2 = setup("/player/attacking/boy_axe_right_2", gp.tileSize * 2, gp.tileSize);
		}
	...
	@Override
	public void update() {
		// Remove these lines from Player.update():
...
//	}
	...
	public void draw(Graphics2D g2) {
		...
		if (invincible) {
			g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, Constants.Player.INVINCIBILITY_ALPHA));
		}
	...
	public void selectItem() {
		...
		if (itemIndex < inventory.size()) {
			...
			if (selectedItem.type == type_sword || selectedItem.type == type_axe) {
				...
				attack = getAttack();
				getPlayerAttackImage(); // Update attack sprites based on new weapon
